...
if [ $last_binlog_name = $binlog_name ]; then
        ...
        mysqlbinlog --no-defaults -v -v /data/mysql/$binlog_name    |grep -E "UPDATE|INSERT|DELETE" |awk '{print $NF}' |sort  |uniq -c |sort -rn |head -100  >> $last_binlog_name_path/$dt.Digest_Binlog.log;
